...
source("fcns/2_1b_model_epidemic_yearcross.R") # downloaded from 
# https://github.com/NaomiWaterlow/NextGen_Flu_Thai/blob/main/Vacc_epi_model/2_1b_model_epidemic_yearcross.R 
...
## checking all n_pos values during epidemics
# n_pos_check <- df_epid_threshold %>% 
#   filter(epidem_inclusion>0) %>% 
#   select(country,ISO_WEEK,value,epid_index,STRAIN,metasource)
# 
# ggplot(n_pos_check[n_pos_check$country %in% c('Argentina', 'Australia','Ghana') & n_pos_check$epid_index < 5,], 
#        aes(x=ISO_WEEK, y=value, group=interaction(epid_index,STRAIN,metasource), 
#            col=interaction(STRAIN,metasource))) + 
#   geom_line() + theme_minimal() + 
#   facet_grid(epid_index~country)
